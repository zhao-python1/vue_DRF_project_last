  ...
  <div class="box">
    ...
    <div class="register">
      <div class="register_box">
        ...
        <div class="inp">
          <input v-model="mobile" @blur="reappera_mobile" type="text" placeholder="手机号码" class="user">
    ...
    methods: {
      user_register() {
        this.$axios({
          ...
          sessionStorage.user_id = response.data_id;
      ...
      get_code() {
        ...
        this.$axios({
          url: this.$settings.HOST + "user/message/" + `${this.mobile}`,
          method: "get",
        }).then(response => {
          console.log(response.data);
          this.sms_flag = true;
          ...
          let timer = setInterval(() => {
            if (interval <= 1) {
              //停止计时器  允许发送短信
              clearInterval(timer);
              this.sms_flag = false; //设置允许发送短信
              this.sms_text = `点击发送短信息`
            } else {
              interval--;
          ...
          console.log(error.response);
          this.$message.error("当前手机号已经发送过短信息")
        })
      }
    },
  }
